...
export class OrderService {
    ...
    async findAll(query: OrderPaginationDTO): Promise<Pagination<OrderResponseDTO>> {
        ...
        if (customer_id) queryBuilder.andWhere('order.customer_id = :customer_id', { customer_id });
        if (restaurant_id) queryBuilder.andWhere('order.restaurant_id = :restaurant_id', { restaurant_id });
        if (driver_id) queryBuilder.andWhere('order.driver_id = :driver_id', { driver_id });
        if (sortBy && sortOrder) queryBuilder.orderBy(`order.${sortBy}`, sortOrder);
        else queryBuilder.orderBy('order.order_date', 'DESC');
        const result = await paginate<Order>(queryBuilder, { page, limit } as IPaginationOptions);
        return { items: plainToInstance(OrderResponseDTO, result.items), meta: result.meta, links: result.links };
    }
    ...
    async create(createDto: CreateOrderDTO, customerId: string): Promise<OrderResponseDTO> {
        ...
        const customer = await this.customerRepository.findOne({ where: { id: createDto.customer_id } });
        if (!customer) throw new CustomerNotFoundException(createDto.customer_id);
        const restaurant = await this.restaurantRepository.findOne({ where: { id: createDto.restaurant_id } });
        if (!restaurant) throw new RestaurantNotFoundException(createDto.restaurant_id);
        if (!restaurant.is_active) throw new RestaurantNotActiveException(createDto.restaurant_id);
        if (!createDto.items || createDto.items.length === 0) throw new EmptyOrderException();

        let totalAmount = 0;
        const orderItemsData: Array<{ menu_item_id: string; quantity: number; unit_price: number; subtotal: number }> = [];

        for (const item of createDto.items) {
            if (item.quantity < 1) throw new InvalidQuantityException('Quantity must be at least 1');
            const menuItem = await this.menuItemRepository.findOne({ where: { id: item.menu_item_id } });
            if (!menuItem) throw new RestaurantNotFoundException(item.menu_item_id);
            if (!menuItem.is_available) throw new MenuItemNotAvailableForOrderException(item.menu_item_id);
            if (menuItem.restaurant_id !== createDto.restaurant_id) throw new InvalidOrderStatusException('creation', 'Menu item does not belong to the specified restaurant');
            const subtotal = Number(menuItem.price) * item.quantity;
            totalAmount += subtotal;
            orderItemsData.push({ menu_item_id: item.menu_item_id, quantity: item.quantity, unit_price: Number(menuItem.price), subtotal });
        }
        ...
        const completeOrder = await this.orderRepository.findOne({ where: { id: savedOrder.id }, relations: ['orderItems'] });
        if (!completeOrder) throw new OrderNotFoundException(savedOrder.id);
    ...
    async update(id: string, updateDto: UpdateOrderDTO): Promise<OrderResponseDTO> {
        ...
        if (order.status === OrderStatus.DELIVERED) throw new OrderAlreadyDeliveredException(id);
        if (updateDto.status && updateDto.status !== order.status) this.validateStatusTransition(order.status, updateDto.status);
        if (updateDto.driver_id) {
            const driver = await this.driverRepository.findOne({ where: { id: updateDto.driver_id } });
            if (!driver) throw new DriverNotFoundException(updateDto.driver_id);
            if (!driver.is_available) throw new DriverNotAvailableException(updateDto.driver_id);
        }
    ...
    async updateStatus(id: string, status: OrderStatus): Promise<OrderResponseDTO> {
        ...
        if (!order) throw new OrderNotFoundException(id);
        if (order.status === OrderStatus.CANCELLED) throw new OrderAlreadyCancelledException(id);
        if (order.status === OrderStatus.DELIVERED && status !== OrderStatus.DELIVERED) throw new OrderAlreadyDeliveredException(id);
    ...
    async assignDriver(orderId: string, driverId: string): Promise<OrderResponseDTO> {
        ...
        if (!driver) throw new DriverNotFoundException(driverId);
        if (!driver.is_available) throw new DriverNotAvailableException(driverId);
