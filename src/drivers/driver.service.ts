...
export class DriverService {
    ...
    async findByEmail(email: string): Promise<DriverResponseDTO> {
        ...
        if (!driver) throw new DriverEmailNotFoundException(email);
        return new DriverResponseDTO(driver);
    }
    ...
    async update(updateDto: UpdateDriverDTO, id: string): Promise<DriverResponseDTO> {
        ...
        return new DriverResponseDTO(await this.driverRepository.save(driver));
    }
    ...
    async updatePassword(updateDto: UpdatePasswordDTO, id: string): Promise<string> {
        ...
        driver.password = await bcrypt.hash(updateDto.newPassword, 10);
        driver.updated_at = new Date();
    ...
    async forgotPassword(forgotPasswordDto: ForgotPasswordDTO): Promise<string> {
        const driver = await this.driverRepository.findOne({ where: { email: forgotPasswordDto.email } });
        if (!driver) throw new DriverEmailNotFoundException(forgotPasswordDto.email);
        if (forgotPasswordDto.newPassword !== forgotPasswordDto.confirmNewPassword) throw new PasswordsNotMatchException();
        driver.password = await bcrypt.hash(forgotPasswordDto.newPassword, 10);
        driver.updated_at = new Date();
        await this.driverRepository.save(driver);
    ...
    async uploadProfileImage(id: string, file: Express.Multer.File): Promise<DriverResponseDTO> {
        ...
        try {
            if (driver.profile_image_url) {
                ...
                if (oldFileName) {
                    await getSupabaseClient().storage.from('profile-banner-images').remove([`drivers/${oldFileName}`]);
                }
            }
            const fileName = `${id}-${Date.now()}-${file.originalname.replace(/\s/g, '-')}`;
            const { error: uploadError } = await getSupabaseClient().storage
                .from('profile-banner-images')
                .upload(`drivers/${fileName}`, file.buffer, { contentType: file.mimetype, upsert: true });
            if (uploadError) throw new FileUploadException(uploadError.message);
            const { data: { publicUrl } } = getSupabaseClient().storage
                .from('profile-banner-images').getPublicUrl(`drivers/${fileName}`);
